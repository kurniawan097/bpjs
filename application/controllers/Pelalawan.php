...
class pelalawan extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('pelalawan_model');
    ...
    public function index() {
        $pelalawan = $this->pelalawan_model->get_all();

        $data = array(
            'pelalawan_data' => $pelalawan
        );

        $this->template->display('pelalawan/tb_pelalawan_list', $data);
    }



    public function read($kd) {
        $row = $this->pelalawan_model->get_by_kd($kd);
        if ($row) {
            $data = array(
            ...
            $this->template->display('pelalawan/tb_pelalawan_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('pelalawan'));
        }
    }
    ...
    public function create() {
        $data = array(
            'button' => 'Create',
            'action' => site_url('pelalawan/create_action'),
        ...
        $this->template->display('pelalawan/tb_pelalawan_form', $data);
    }
    ...
    public function create_action() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->pelalawan_model->insert($data);
    ...
    public function update_action() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->pelalawan_model->update($this->input->post('kode_faskes', TRUE), $data);
    ...
    public function delete($kd) {
        ...
        if ($row) {
            $this->pelalawan_model->delete($kd);
    ...
    public function _rules() {
     ...
     function excel() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E2', "KABUPATEN PELALAWAN");   
        ...
        $no = 1;
        $counter = 5;
        foreach ($this->pelalawan_model->get_all() as $data):
        ...
        $objPHPExcel->getActiveSheet()->setTitle('kab pelalawan');
        ...
        header('Content-Disposition: attachment;filename="Faskes Kab Pelalawan'. date('dmY') .'.xlsx"');
...
/* End of file pelalawan.php */
/* Location: ./application/controllers/pelalawan.php */
