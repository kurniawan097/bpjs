...
class rohul extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('rohul_model');
    ...
    public function index() {
        $rohul = $this->rohul_model->get_all();

        $data = array(
            'rohul_data' => $rohul
        );

        $this->template->display('rohul/tb_rohul_list', $data);
    }



    public function read($kd) {
        $row = $this->rohul_model->get_by_kd($kd);
        if ($row) {
            $data = array(
            ...
            $this->template->display('rohul/tb_rohul_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('rohul'));
        }
    }
    ...
    public function create() {
        $data = array(
            'button' => 'Create',
            'action' => site_url('rohul/create_action'),
        ...
        $this->template->display('rohul/tb_rohul_form', $data);
    }
    ...
    public function update($kd) {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('rohul/update_action'),
    ...
    public function update_action() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->rohul_model->update($this->input->post('kode_faskes', TRUE), $data);
    ...
    function excel() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E2', "KABUPATEN ROKAN HULU");   
        ...
        $no = 1;
        $counter = 5;
        foreach ($this->rohul_model->get_all() as $data):
        ...
        $objPHPExcel->getActiveSheet()->setTitle('kab rohul');
        ...
        header('Content-Disposition: attachment;filename="Faskes Kab Rohul'. date('dmY') .'.xlsx"');
        
        $objWriter->save('php://output');
    }
}

/* End of file Rohul.php */
/* Location: ./application/controllers/Rohul.php */
