...
class fpktwo extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('fpktwo_model');
    ...
    public function index() {
        $fpktwo = $this->fpktwo_model->get_all();

        $data = array(
            'fpktwo_data' => $fpktwo
        );

        $this->template->display('fpktwo/tb_fpktwo_list', $data);
    }
    ...
    function get_faskes(){
        ...
        $data=$this->fpktwo_model->get_faskes_bykode($kode_faskes);
    ...
    function get_program(){
        ...
        $data=$this->fpktwo_model->get_faskes_bynama($uraian);
        echo json_encode($data);
    }

    //fungsi memanggil tabel jejaring
    function get_jejaring(){
        $nama_jejaring=$this->input->post('nama_jejaring');
        $data=$this->fpktwo_model->get_byjaring($nama_jejaring);
        echo json_encode($data);
    }

    public function read($id) {
        $row = $this->fpktwo_model->get_by_id($id);
        if ($row) {
            $data = array(
                'kode_faskes' => $row->kode_faskes,
                'nama_faskes' => $row->nama_faskes,
            );
            $this->template->display('fpktwo/tb_fpktwo_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('fpktwo'));
        }
    }
    ...
    public function create() {
        $data = array(
            'button' => 'Create',
            'action' => site_url('fpktwo/create_action'),
          ...
          //  'id_fpktwo'=> set_value('id_fpktwo'),
            'id_fpk'=> set_value('id_fpk'),
           
        );
        
        $this->template->display('fpktwo/tb_fpktwo_form', $data);
    }
    ...
    public function create_action() 
        {
            $data = array(
               ...
               // 'id_fpktwo'=> $this->input->post('id_fpktwo'));
                $id_fpktwo =$this->fpktwo_model->insert($data);
                ...
                'uraian' => $this->input->post('uraian'),
                'id_fpk'=> $id_fpktwo);
                $proses = $this->fpktwo_model->tambah_peserta($data_peserta);   */
                ...
                $proses = $this->fpktwo_model->tambah_peserta($result);
    ...
    public function _rules() {
        ...
        $this->form_validation->set_rules('noka', 'noka', 'trim|required');
    ...
    public function excel($id) {
        $row = $this->fpktwo_model->get_by_id($id);
        $select = $this->db->get('tb_fpktwo')->result();
       ...
       $style_one = array(      
        ...
        foreach($this->fpktwo_model->get_fpktwo($id) as $data){
            ...
            $excel->setActiveSheetIndex(0)->setCellValue('E12', $data->nama_jejaring);
        ...
        foreach($this->fpktwo_model->get_kepala() as $data){
        ...
        $counter = 13;
        foreach($this->fpktwo_model->get_peserta($id) as $data):
       ...
       // $excel->setActiveSheetIndex(1)->setCellValue('L'.$n, "Anggi Pramana");
        ...
        foreach($this->fpktwo_model->get_pra() as $data){
        $excel->setActiveSheetIndex(1)->setCellValue('E'.$n, $data->nama_pegawai);
        }

        foreach($this->fpktwo_model->get_keu() as $data){
        $excel->setActiveSheetIndex(1)->setCellValue('J'.$n, $data->nama_pegawai);
        }

        foreach($this->fpktwo_model->get_verif() as $data){
        ...
        $excel->getActiveSheet()->setTitle('Jejaring');
        ...
        $objDrawing->setName('Bpjs Kesehatan');
        $objDrawing->setDescription('Logo BPJS Kesehatan');
        $objDrawing->setPath('C:/xampp/htdocs/bpjs/assets/img/logo_3.png');
        $objDrawing->setCoordinates('B2'); 
        $objDrawing->setWorksheet($excel->getActiveSheet());

        //kop
        $excel->setActiveSheetIndex(2)->setCellValue('E2', "Rekapan Pengajuan Klaim");   
        $excel->getActiveSheet()->mergeCells('E2:F2'); 
        $excel->setActiveSheetIndex(2)->setCellValue('E3', "Bulan Pelayanan");
        $excel->setActiveSheetIndex(2)->setCellValue('E4', "Jenis Pelayanan:");
        $excel->getActiveSheet()->getStyle('B2:K5')->applyFromArray($style_end2); 

        //Tabel Peserta
        $excel->setActiveSheetIndex(2)->setCellValue('B7', "No");
        $excel->getActiveSheet()->mergeCells('B7:B8');
        $excel->setActiveSheetIndex(2)->setCellValue('C7', "Nama Jejaring");
        $excel->getActiveSheet()->mergeCells('C7:C8');
        $excel->setActiveSheetIndex(2)->setCellValue('D7', "Nama Peserta");
        $excel->getActiveSheet()->mergeCells('D7:D8');
        $excel->setActiveSheetIndex(2)->setCellValue('E7', "NOKA");
        $excel->getActiveSheet()->mergeCells('E7:E8');
        $excel->setActiveSheetIndex(2)->setCellValue('F7', "Tanggal Pelayanan");
        $excel->getActiveSheet()->mergeCells('F7:G7');
        $excel->setActiveSheetIndex(2)->setCellValue('F8', "Tgl Masuk");
        $excel->setActiveSheetIndex(2)->setCellValue('G8', "Tgl Keluar ");
        $excel->setActiveSheetIndex(2)->setCellValue('H7', "TELAAHAN VERIFIKASI");
        $excel->getActiveSheet()->mergeCells('H7:K7');
        $excel->setActiveSheetIndex(2)->setCellValue('H8', "Diajukan");
        $excel->setActiveSheetIndex(2)->setCellValue('I8', "Jumlah");
        $excel->setActiveSheetIndex(2)->setCellValue('J8', "Nama/No Rekening");
        $excel->setActiveSheetIndex(2)->setCellValue('K8', "No. NPWP");
        $excel->setActiveSheetIndex(2)->setCellValue('B9', "1");
        $excel->getActiveSheet()->mergeCells('B9:B11');
        $excel->getActiveSheet()->getStyle('B7:K8')->applyFromArray($style_grs);
        $excel->getActiveSheet()->getStyle('J8:K8')->applyFromArray($style_atki);
        $excel->getActiveSheet()->getStyle('I8:J8')->applyFromArray($style_atka);
        $excel->getActiveSheet()->getStyle('H8:H8')->applyFromArray($style_atka);
        $excel->getActiveSheet()->getStyle('F8:F8')->applyFromArray($style_atka);
        $excel->getActiveSheet()->getStyle('G8:G8')->applyFromArray($style_atka);
        $excel->getActiveSheet()->getStyle('F7:G7')->applyFromArray($style_kn);
        $excel->getActiveSheet()->getStyle('B7:B8')->applyFromArray($style_kn);
        $excel->getActiveSheet()->getStyle('C7:C8')->applyFromArray($style_kn);
        $excel->getActiveSheet()->getStyle('D7:D8')->applyFromArray($style_kn);
        $excel->getActiveSheet()->getStyle('E7:E8')->applyFromArray($style_kn);
        
        foreach($this->fpktwo_model->get_fpktwo($id) as $data){
            $excel->setActiveSheetIndex(2)->setCellValue('G2', $data->nama_faskes);
            $excel->getActiveSheet()->mergeCells('G2:K2');
            $excel->setActiveSheetIndex(2)->setCellValue('F3', $data->bulan_pelayanan);
            $excel->getActiveSheet()->mergeCells('F3:I3');
            $excel->setActiveSheetIndex(2)->setCellValue('F4', $data->uraian_prog);
        ...
        $counter = 9;
        foreach($this->fpktwo_model->get_peserta($id) as $data):
            $excel->setActiveSheetIndex(2)->setCellValue('D'. $counter, $data->nama_peserta); 
            $excel->setActiveSheetIndex(2)->setCellValue('E'. $counter, $data->noka);
            $excel->setActiveSheetIndex(2)->setCellValue('F'. $counter, $data->tanggal_masuk);
            $excel->setActiveSheetIndex(2)->setCellValue('G'. $counter, $data->tanggal_keluar);
            $excel->setActiveSheetIndex(2)->setCellValue('H'. $counter, $data->diajukan);
        ...
        foreach($this->fpktwo_model->get_fpktwo($id) as $data){
        $excel->setActiveSheetIndex(2)->setCellValue('H'.$ct, $data->jumlah_ajukan);
        $excel->setActiveSheetIndex(2)->setCellValue('I9', $data->jumlah_ajukan);
        $excel->getActiveSheet()->mergeCells('I9:I11');
        $excel->setActiveSheetIndex(2)->setCellValue('I'.$ct, $data->jumlah_ajukan);
        $excel->setActiveSheetIndex(2)->setCellValue('C9', $data->nama_jejaring);
        $excel->getActiveSheet()->mergeCells('C9:C11');
        $excel->setActiveSheetIndex(2)->setCellValue('J9', $data->norek);
        $excel->getActiveSheet()->mergeCells('J9:J11');
        $excel->setActiveSheetIndex(2)->setCellValue('K9', $data->nonpwp);
        $excel->getActiveSheet()->mergeCells('K9:K11');
        }

        //footer nya lah, wkwk
        $excel->setActiveSheetIndex(2)->setCellValue('D'.$ct, "Jumlah.");
        $excel->setActiveSheetIndex(2)->setCellValue('J'.$cv, "Verifikator");
        //$excel->setActiveSheetIndex(2)->setCellValue('J'.$cl, "Anggi Pramana");

        foreach($this->fpktwo_model->get_verif() as $data){
        $excel->setActiveSheetIndex(2)->setCellValue('J'.$cl, $data->nama_pegawai);
        }



        //style Tabel
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle9, "C9:C10");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle9, "B9:B$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle9, "C11:C$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle4, "D9:D$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle1, "E9:G$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle5, "H9:H$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStylekk, "I9:I$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyleend, "J9:K$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle8, "C$ct:G$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle5, "H$ct:I$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle5, "J$ct:K$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStyle5, "B$ct");
        $excel->getActiveSheet(2)->setSharedStyle($sharedStylef, "J$cv:J$cl");
        ...
        $excel->getActiveSheet()->getColumnDimension('C')->setWidth(19); // Set width kolom C
...
/* End of file fpktwo.php */
/* Location: ./application/controllers/fpktwo.php */
