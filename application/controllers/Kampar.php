...
class kampar extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('kampar_model');
    ...
    public function index() {
        $kampar = $this->kampar_model->get_all();

        $data = array(
            'kampar_data' => $kampar
        );

        $this->template->display('kampar/tb_kampar_list', $data);
    }



    public function read($kd) {
        $row = $this->kampar_model->get_by_kd($kd);
        if ($row) {
            $data = array(
            ...
            $this->template->display('kampar/tb_kampar_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('kampar'));
        }
    }
    ...
    public function create() {
        $data = array(
            'button' => 'Create',
            'action' => site_url('kampar/create_action'),
        ...
        $this->template->display('kampar/tb_kampar_form', $data);
    }
    ...
    public function update($kd) {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('kampar/update_action'),
    ...
    public function update_action() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->kampar_model->update($this->input->post('kode_faskes', TRUE), $data);
    ...
    public function _rules() {
     ...
     function excel() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E2', "KABUPATEN KAMPAR");   
        ...
        $no = 1;
        $counter = 5;
        foreach ($this->kampar_model->get_all() as $data):
        ...
        $objPHPExcel->getActiveSheet()->setTitle('kab kampar');
        ...
        header('Content-Disposition: attachment;filename="Faskes Kab Kampar'. date('dmY') .'.xlsx"');
...
/* End of file kampar.php */
/* Location: ./application/controllers/Kampar.php */
