...
class Fpktwo extends CI_Controller {
    ...
    private static $table = 'tb_fpkone';
    private static $primaryKey = 'id_fpkone';

    public function index()
	{
        $data['title'] = "Data ".self::$title;
        $data['content'] = "fpktwo/fpktwo-view";
		$this->load->view('fpktwo/fpktwo-view', $data);
	}
    ...
    public function get_data()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $columns = array(
            array('db' => 'kode_faskes', 'dt' => 'kode_faskes'),
            array('db' => 'nama_faskes', 'dt' => 'nama_faskes'),
            array('db' => 'bulan_pelayanan', 'dt' => 'bulan_pelayanan'),
            array('db' => 'tgl_masuk', 'dt' => 'tgl_masuk'),
            array('db' => 'tgl_masuk', 'dt' => 'tgl_masuk'),
            array('db' => 'noreg_masuk', 'dt' => 'noreg_masuk'),
            array('db' => 'tgl_yankes', 'dt' => 'tgl_yankes'),
            array('db' => 'noreg_yankes', 'dt' => 'noreg_yankes'),
            array('db' => 'jenis_penagihan', 'dt' => 'jenis_penagihan'),
            array('db' => 'jenis_pelayanan', 'dt' => 'jenis_pelayanan'),
            array('db' => 'jenis_pelayanan', 'dt' => 'jenis_pelayanan'),
            array('db' => 'nama_jejaring', 'dt' => 'nama_jejaring'),
            array('db' => 'nama_program', 'dt' => 'nama_program'),
            array('db' => 'kode_program', 'dt' => 'kode_program'),
            array('db' => 'kode_akun', 'dt' => 'kode_akun'),
            array('db' => 'uraian_biaya', 'dt' => 'uraian_biaya'),
            array('db' => 'kasus_ajukan', 'dt' => 'kasus_ajukan'),
            array('db' => 'jumlah_ajukan', 'dt' => 'jumlah_ajukan'),
            array('db' => 'kasus_setuju', 'dt' => 'kasus_setuju'),
            array('db' => 'jumlah_setuju', 'dt' => 'kasus_setuju'),
            array('db' => 'id_fpkone', 'dt' => 'id_fpkone'),
                
                array(
                    'db' => 's_id',
                    'dt' => 'tindakan',
                    'formatter' => function($id_fpkone) {
                        return '<a class="btn btn-warning btn-sm mb" href="'.site_url('fpktwo/print_data/'.$id_fpkone).'" target="_blank" title="Cetak"><span class="glyphicon glyphicon-print" aria-hidden="true"></span></a>
                        <a class="btn btn-success btn-sm mb" href="'.site_url('fpktwo/view/'.$id_fpkone).'">Lihat</a>
                        <a class="btn btn-info btn-sm mb" href="'.site_url('fpktwo/edit/'.$s_id).'">Ubah</a>
                        <a class="btn btn-danger btn-sm mb" onclick="return confirmDialog();" href="'.site_url('student/delete/'.$id_fpkone).'" title="Hapus"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a><a class="btn btn-default btn-sm mb" href="'.site_url('student/status/'.$id_fpkone).'" onclick="return confirmDialogStatus();">Ubah Status</a>';
                    }
                ),
            );
